...
class DensityHNN(object):
    ...
    def run(self,
        ...
        if expanding: # performs __static_fit() for each expanding window.
            ...
            results = self.__static_fit(data, train_start=train_start, 
                                      train_end=train_end)
    ...
    def __static_fit(self, data:Union[pd.DataFrame, pd.Series],
                   train_start:Union[str, datetime],
                   train_end:Union[str, datetime]):
        ...
        hnn_prediction = ensemble_hnn.predict(
        ...
        oob_idx = ensemble_hnn.oob_indices
